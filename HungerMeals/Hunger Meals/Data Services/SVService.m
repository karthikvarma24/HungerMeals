...
-(void)getLocationsDataUsingBlock:(void(^)(id result, NSHTTPURLResponse *response, NSError *err))block{
    ...
    [self sendGetRequestWithAuth:url usingblock:^(id result, NSHTTPURLResponse *response, NSError *err) {
        if (response.statusCode == 200 && result!=nil) {
            ...
            block((id)dictResult,response,err);
        }
        else{
            
            
            block((id)result,response,err);
        }
    }];
...
- (void)getUpdateLocationsDataUsingBlock:(void(^)(NSMutableDictionary *resultArray))resultBlock{
    ...
    NSString *token = userDataObject.token;
    NSString *url = [NSString stringWithFormat:kUpdatedLocation, HTTP_DATA_HOST,token];
    [self sendGetRequestWithAuth:url usingblock:^(id result, NSHTTPURLResponse *response, NSError *err) {
        if (response.statusCode == 200 && result!=nil) {
    ...
    [self sendRequest:url Perameters:params usingblock:^(id result, NSHTTPURLResponse *response, NSError *err) {
        ...
        if (response.statusCode == 200 && result!=nil) {
            ...
            id dictResult = [NSJSONSerialization JSONObjectWithData:result options:NSJSONReadingAllowFragments error:nil];
              ...
              [self performSelectorOnMainThread:@selector(showActivityIndicatorWithTitle:) withObject:kIndicatorTitleSaving waitUntilDone:NO];
