...
- (void)viewDidLoad {
   ...
   // self.shareLocationButton .layer.cornerRadius = 2; // this value vary as per your desire
     //self.shareLocationButton.clipsToBounds = YES;
...
- (void) saveLocation
{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys: selectedAddressType, @"addresstype", self.cityTextField.text, @"name", self.cityTextField.text, @"city", self.areaLocalityTextField.text, @"sublocation",  self.flatNumberTextField.text, @"address", [NSString stringWithFormat:@"%f",latitudeValue],  @"lat", [NSString stringWithFormat:@"%f",longitudeValue], @"lng", self.pinCodeTextField.text, @"zip", @"userlocation", @"type", nil];
